...
<script>
        jQuery(function($){
                ...
                $("#bookingTable_in").DataTable({   "order": [],"language": { <?php if(isset($_GET['type']) && $_GET['type'] == 'completed'){ ?>emptyTable: "No tutoring sessions have been completed"<?php }else{ ?>emptyTable: "No tutoring packages have been purchased"<?php } ?>,search: "",searchPlaceholder: "Search..." },"dom": '<"row"<"col-sm-2"l><"col-sm-4"f><"col-sm-4"<"bwtoolfilter">><"col-sm-2"<"bwtoolbook">>>rtip', fixedHeader: {
                    header: true,
                    footer: true
...
$('#clearbtn').click(function() {
    ...
    $("div.bwtoolfilter").html("<select id='bookdet' style='border: 1px solid #ccc; padding: 4px 10px; width: 100%!important; background: #fff!important; border-radius: 5px;'><option <?php if(isset($_GET['type']) && $_GET['type'] == 'upcoming') { echo "selected"; } ?> value='upcoming'>Current/Upcoming</option><option <?php if(isset($_GET['type']) && $_GET['type'] == 'completed') { echo "selected"; } ?> value='completed'>Completed</option></select>");
